...
async function execute(bot, playerId, args) {
    try {
        ...
        if (claimSuccess) {
            ...
            if (!errorFlag && (coinAmount > 0 || emeraldAmount > 0)) {
                ...
                // TODO: 要加到錢包
                bot.chat(
                    `成功領取[${dailyReward.rankName}]的每日獎勵: ` +
        ...
        Logger.error(`[daily] ${playerId} 領取每日獎勵失敗:`, error);
        bot.chat(`/m ${playerId} &c領取每日獎勵時發生錯誤: ${error.message}`);
    }
}